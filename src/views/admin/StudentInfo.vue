...
import { QueryForm, StudentForm } from "@/types/studentTypes.ts";
...
const tableData = ref<StudentForm[]>([]);
...
const addButton = () => {
  Object.keys(addForm).forEach((key) => {
    if (key === "sex") {
      addForm[key] = null; // 保持 sex 字段为 null
    } else {
...
// “编辑”按钮
const editButton = (row: StudentForm) => {
  editFormVisible.value = true;
  handleEchoData(row);
};

// “删除”按钮
const deleteButton = (row: StudentForm) => {
...
// 删除学生信息
async function handleDeleteStudentInfo(userId: number) {
  try {
